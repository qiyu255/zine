// file: CalendarMonthMiniView.tsx
import './Calendar.css'
...
interface DayGridProps {
  cells: ReturnType<typeof buildCalendarData>;
  today: Date;
}
...
// ----- 主组件 -----
export default function CalendarMonthMiniView({ date }: CalendarMonthViewProps) {
  const year = date.getFullYear();
  const month = date.getMonth();
  const today = new Date();

  const cells = buildCalendarData(year, month);
  const start = cells[0].date
  const end = cells[cells.length - 1].date as Date
  end.setTime(end.getTime() + 86400000)
  // getCalendarUserdata(start,end)
